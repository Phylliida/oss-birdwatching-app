// iNat photos. Usage:  TAXON=oaks node scripts/taxon/05-fetch-inat.mjs
...
const OUT_PATH = `${taxon.dataDir}/inat.json`;
const CONCURRENCY = 1;
...
async function lookup(name) {
  ...
  const photos = (tt.taxon_photos || [])
    ...
    .slice(0, TOP_N)
    .map((p) => ({ url: (p.medium_url || p.url).replace(/^http:\/\//, "https://"), attribution: p.attribution || null, license: p.license_code || null }));
...
for (const sp of species) {
  ...
  try { const r = await lookup(sp.scientificName); existing[sp.scientificName] = r; dirty = true; if (r.photos.length) found++; }
