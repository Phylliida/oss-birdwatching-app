// Wikipedia summaries.  TAXON=oaks node scripts/taxon/03-fetch-wikipedia.mjs
...
const OUT_PATH = `${taxon.dataDir}/wiki.json`;
const CONCURRENCY = 4;
...
if (existsSync(OUT_PATH)) {
  ...
  catch { console.warn("Couldn't parse cache; starting fresh."); }
}
...
async function fetchSummary(name) {
  ...
  for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
    try {
      ...
      if (res.status === 429 || res.status >= 500) { await new Promise((r) => setTimeout(r, 1000 * Math.pow(2, attempt))); continue; }
...
async function worker() {
  while (nextIdx < species.length) {
    ...
    } catch (err) { errors++; if (errors <= 5) console.warn(`  Error for ${name}: ${err.message}`); }
    done++;
    if (done % 100 === 0) {
      if (dirty) { await writeFile(OUT_PATH, JSON.stringify(existing)); dirty = false; }
      const rate = done / ((Date.now() - start) / 1000);
      process.stdout.write(`  ${done}/${species.length} | ${found} found | ${rate.toFixed(1)}/s | ETA ${((species.length - done) / rate / 60).toFixed(1)}min\n`);
    }
...
console.log(`Done. ${Object.values(existing).filter(v => v).length}/${species.length} with summary`);
