...
for (let i = 0; i < species.length; i += BATCH) {
  ...
  // Skip whole batches whose species are all already cached.
  if (batch.every((s) => s.scientificName in data)) continue;
  let bindings;
  try { bindings = await queryBatch(batch); }
  catch (err) {
    console.warn(`Batch ${i} failed permanently: ${err.message}; saving progress and re-throwing`);
    if (dirty) await writeFile(OUT_PATH, JSON.stringify(data));
    throw err;
  }
