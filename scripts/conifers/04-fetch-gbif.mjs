// Fetches GBIF country/state observation distribution for each conifer
// species. Uses the speciesKey we already have from the species-list fetch,
// so it's a single occurrence/search request per species.

import { readFile, writeFile } from "node:fs/promises";
import { existsSync } from "node:fs";

const SPECIES_PATH = "data/conifers/species.json";
const OUT_PATH = "data/conifers/gbif.json";
const CONCURRENCY = 2;
...
async function worker() {
  while (nextIdx < species.length) {
    ...
    try {
      const r = await fetchOne(sp.gbifKey);
      existing[sp.scientificName] = r;
      dirty = true;
      if (r.total > 0) found++; else missing++;
    } catch (err) {
      errors++;
    ...
    if (done % 100 === 0) {
      ...
      process.stdout.write(`  ${done}/${species.length} | ${found} with data | ${rate.toFixed(1)}/s | ETA ${eta}min\n`);
    }
...
console.log(`Done. ${found} with data, ${missing} empty, ${errors} errors`);
